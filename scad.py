...
def get_postcard_bottom(thing, **kwargs):
    ...
    #add countersunk_screws
    if False:
        positions = []
        shift_x = (width-1)/2 *15
        shift_y = (height-1)/2 *15
        shift_z = depth
        positions.append( [-shift_x, -shift_y, shift_z] )
        positions.append( [shift_x, -shift_y, shift_z] )
        positions.append( [-shift_x, shift_y, shift_z] )
        p3 = copy.deepcopy(kwargs)
        p3["type"] = "negative"
...
def get_postcard_corner(thing, **kwargs):
